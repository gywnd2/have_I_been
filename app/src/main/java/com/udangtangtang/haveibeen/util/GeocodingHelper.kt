...
class GeocodingHelper(private val context: Context) {
    ...
    private lateinit var geocoderListener : GeocodeListener
    ...
    init {
        ...
        geocoderListener= GeocodeListener {
            if (it.isNotEmpty()) {
    ...
    fun getAddress(latitude : Double, longtitude: Double): String {
        try {
            // Geocoder를 통해 주소 획득
            addressList = geocoder.getFromLocation(latitude, longtitude, 10, geocoderListener) as java.util.ArrayList<Address>
        } catch (e: IOException) {
